...
app.MapPost("/api/caption", async (IFormFile file, IFormFile srtFile) =>
{
    ...
    try
    {
        ...
        return await GenerateCaptionsZipAsync(tempMp4, file.FileName ?? "video.mp4", srtContent);
    }
    finally
    {
...
app.MapPost("/api/caption/youtube", async (string url) =>
{
    ...
    try
    {
        // 1. Get YouTube video metadata in JSON format
        var (metaExit, metaStdout, metaStderr) = RunProcessWithResult("yt-dlp", $"-j --skip-download \"{url}\"");
        if (metaExit != 0)
        {
            return Results.BadRequest(new { Error = "Failed to retrieve YouTube video metadata. Make sure the URL is valid.", Details = metaStderr });
        }

        string targetLang = "en";
        bool hasManualEn = false;
        try
        {
            using var doc = JsonDocument.Parse(metaStdout);
            var root = doc.RootElement;
            
            if (root.TryGetProperty("subtitles", out var subtitlesElement) && subtitlesElement.ValueKind == JsonValueKind.Object)
            {
                foreach (var prop in subtitlesElement.EnumerateObject())
                {
                    if (prop.Name == "en" || prop.Name.StartsWith("en-", StringComparison.OrdinalIgnoreCase))
                    {
                        hasManualEn = true;
                        targetLang = prop.Name;
                        break;
                    }
                }
            }
        ...
        {
            return Results.BadRequest(new { Error = $"Failed to parse video metadata: {ex.Message}" });
        }

        if (!hasManualEn)
        {
            return Results.BadRequest(new { Error = "Validation Failed: The YouTube video must have manually created English subtitles (auto-generated subtitles are not supported)." });
        }

        // 2. Download video (H.264 MP4) and subtitles (SRT) in one command
        var baseName = Guid.NewGuid().ToString();
        var tempVideoBase = Path.Combine(tempDir, baseName);
        
        var ytDlpArgs = $"-f \"bestvideo[vcodec*=avc]+bestaudio[acodec*=mp4a]/best[ext=mp4]/best\" --merge-output-format mp4 --ffmpeg-location \"{Env.FFMPEG_PATH}\" --write-sub --sub-lang {targetLang} --sub-format srt -o \"{tempVideoBase}\" \"{url}\"";
        
        var (dlExitCode, dlStdout, dlStderr) = RunProcessWithResult("yt-dlp", ytDlpArgs);
        if (dlExitCode != 0)
        {
            return Results.BadRequest(new { Error = "Failed to download video or subtitles from YouTube.", Details = dlStderr });
        }

        var videoPath = tempVideoBase + ".mp4";
        if (!File.Exists(videoPath))
        {
            return Results.BadRequest(new { Error = "Failed to download merged MP4 video file from YouTube." });
        }

        var srtFiles = Directory.GetFiles(tempDir, $"{baseName}.*.srt");
        if (srtFiles.Length == 0)
        {
            return Results.BadRequest(new { Error = "Failed to download manual English subtitles from YouTube." });
        }
        var srtPath = srtFiles[0];
        var srtContent = await File.ReadAllTextAsync(srtPath, Encoding.UTF8);

        // 3. Process video and subtitle content, then generate the same ZIP file as the first API
        return await GenerateCaptionsZipAsync(videoPath, "video.mp4", srtContent);
    }
    catch (Exception ex)
    {
        return Results.Problem($"An error occurred during subtitle processing: {ex.Message}");
    }
    finally
    {
...
static async Task<IResult> GenerateCaptionsZipAsync(string tempMp4, string videoFileName, string srtContent)
{
    ...
    try
    {
        ...
        using (var archive = new ZipArchive(zipMs, ZipArchiveMode.Create, leaveOpen: true))       
        {
            ...
            // corrected_max_3_words_bounce.ass
            ...
            // video file
            var entry3 = archive.CreateEntry(videoFileName);
...
static (int ExitCode, string StandardOutput, string StandardError) RunProcessWithResult(string cmd, string args)
{
